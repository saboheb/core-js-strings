...
function countVowels(str) {
  const volwelsArr = str.match(/[aeiouyAEIOUY]/gi);
  return volwelsArr === null ? 0 : volwelsArr.length;
}
...
function isPalindrome(str) {
  const strToLower = str.toLowerCase().replace(/[ ,.!?]/g, '');
...
function reverseWords(str) {
  return str
    .split(' ')
    .map((index) => index.split('').reverse().join(''))
    .join(' ');
}
...
function encodeToRot13(str) {
  return str.replace(
    ...
    (index) =>
      'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'[
        'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'.indexOf(index)
      ]
  );
}
